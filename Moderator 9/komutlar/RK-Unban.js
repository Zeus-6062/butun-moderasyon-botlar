...
module.exports.run = async (client, message, args) => {
  ...
  if(kisi) {
    ...
    if(ayar.banlog && client.channels.cache.has(ayar.banlog)) client.channels.cache.get(ayar.banlog).send(new MessageEmbed().setColor('RANDOM').setTimestamp().setTitle('Ban Kaldırıldı!').setDescription(`**Kaldıran Yetkili:** ${message.author} (${message.author.id})\n**Banı Kaldırılan Üye:** ${kisi.tag} (${kisi.id})`));
  } else {
