...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!client.config.yetkilialımdm.some(id => message.member.roles.cache.has(id))&& (!message.member.hasPermission("ADMINISTRATOR"))) {
        ...
        if (!member) return message.channel.send(embed.setDescription("Lütfen bir kullanıcı etiketleyip tekrar deneyiniz.\n `.yetkiver @Ducky/ID`")).then(message.react(client.config.no));

        await message.guild.members.cache.get(member.id).roles.add(client.config.ilkyetkilipermi).then(message.react(client.config.tik));
        message.channel.send(embed.setDescription(`${member} adlı kullanıcı'ya **Başlangıç Yetkileri** verildi!`)
