...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!member) return message.channel.send(embed.setDescription("Lütfen bir kullanıcı etiketleyip tekrar deneyiniz.\n `.vocal @Ducky/ID`")).then(message.react(client.config.no));

        await message.guild.members.cache.get(member.id).roles.add(client.config.vocal).then(message.react(client.config.tik));
        message.channel.send(embed.setDescription(`${member} adlı kullanıcı'ya **VOCAL** rolü verildi!`)
