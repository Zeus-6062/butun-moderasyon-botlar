...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!reason || reason.split('.').join('').length < 2) return message.channel.send(embed.setDescription("Kullanıcıyı susturmak için bir neden ve süre yazınız! \n `.mute @Ducky/ID 5m Küfür`")).then(x => x.delete({ timeout: 5000 })).then(message.react(client.config.no));
        let time = args[1]
        let mutezaman = ms(`${time}`)

        db.push(`${member.id}_sicil`, `\`[CHAT MUTE]\` ${message.author} tarafından **${reason}** sebebiyle **${moment().format('LLL')}** tarihinde **${time}** süresince **__yazı__** kanallarında susturuldu.`)
        db.add(`${message.author.id}_chatmute`, 1)
        member.roles.add(client.config.muteRoles)
        message.channel.send(embed.setDescription(`${member} adlı kullanıcı **yazı** kanallarında **${reason}** sebebiyle **${time}** süresince susturuldu`)).then(x => x.delete({ timeout: 5000 })).then(message.react(client.config.tik)).then(() => {
            ...
            if (time) setTimeout(() => {
                ...
                channel.send(embed.setTitle('Kullanıcı Susturması Açıldı!').setDescription(`${member} Adlı Kullanıcının text mutesi açıldı!`))
            }, mutezaman)
        })
    }
    }
