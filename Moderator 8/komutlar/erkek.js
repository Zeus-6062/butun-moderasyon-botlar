...
module.exports.run  = async (client ,message,args ) => {
  ...
  if (!cfg.yetki.register.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(new MessageEmbed().setDescription(`✖️ Bu komutu kullanmak için gerekli yetkiye sahip değilsin!`).setColor(client.randomColor()).setAuthor(message.author.tag, message.member.user.avatarURL({ dynamic: true })).setFooter(client.user.username,client.user.avatarURL({dynamic:true}))).then(msg => msg.delete({timeout: 5000}), message.react(cfg.react.red))
  ...
  try {
  if (yecep.user.bot) return message.channel.send(vegasembed.setDescription(`Botlara herhangi bir işlem uygulayamazsın.`)).then(msg => msg.delete({timeout: 5000}), message.react(cfg.react.red)).catch(err => message.channel.send(new MessageEmbed().setDescription(`Lütfen bir üyeyi etiketle ve tekrar dene!`).setAuthor(message.author.tag, message.member.user.avatarURL({dynamic: true})).setColor('RANDOM')).then(msg => msg.delete({timeout: 5000}), message.react(cfg.react.red)))
  ...
  await yecep.roles.add(cfg.kayıt.erkek)
  await yecep.roles.remove(cfg.kayıt.kadın)   
  ...
  db.add('isimlererkek.'+yecep.id, 1)
  db.push("isimler." + yecep.id,  `\`${yecep.displayName}\` (<@&${cfg.kayıt.erkek[0]}>)\n`)
  ...
  .setDescription(`${yecep} adlı kullanıcı başarıyla erkek olarak kaydedildi.`)
...
exports.conf = {
    enabled: true,
    guildOnly: true,
    aliases: ["e","man","adam","Erkek","ERKEK","E"],
    permLevel: 0,
  }
