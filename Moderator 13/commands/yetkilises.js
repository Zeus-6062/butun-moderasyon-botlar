...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        let sesteolmayan = message.guild.members.cache.filter(s => s.roles.cache.has('817019944725774401')).filter(s => !s.voice.channel).map(s => s).join(' ')
        let sesteolan = message.guild.members.cache.filter(s => s.roles.cache.has('817019944725774401')).filter(s => s.voice.channel).map(s => s).join(', ')
