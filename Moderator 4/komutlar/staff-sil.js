...
let embed = new MessageEmbed().setColor("0x2f3136")
    ...
    message.channel.bulkDelete(Number(args[0])).catch(c => console.log("[CLEAR] Unknown Message")).then(msjlar => message.channel.send(`${ayarlar.yeah} Başarıyla <#${message.channel.id}> adlı kanalda **${msjlar.size}** adet mesaj silindi!`).then(x => x.delete({timeout: 5000 })))
